...
void PIT0_IRQHandler(void)
{
 ...
 //  if((allow_quickstop_flag==1 || endflag) && !zdnum)
   if((quickstop_flag==1 || endflag) && !zdnum)
   {
       ...
       //     while(currentspeed>10);
           if(currentspeed<20 && quickstop_flag==1) //终点时直接占空比给0
           {
         //      pit_delay_ms(PIT1, 200);//短暂延时
       //  allow_quickstop_flag=2;
               quickstop_flag=2;
               fznum=80; //反转场数重定义给终点急停
               zdnum=20;//识别终点后给予一定场数的延时           
           }
        }
   }
 ...
 /*     if(motorPWM-last_motorPWM>=50 ) //超过50至慢慢加/减
        motorPWM=last_motorPWM+50;
      else if(last_motorPWM-motorPWM>=50 )
        motorPWM=last_motorPWM-50;
       ftm_pwm_duty(FTM0, FTM_CH2, motorPWM);
       ftm_pwm_duty(FTM0, FTM_CH3, 0);
      last_motorPWM=motorPWM;//上一次pwm值
  */ 
   }
/*    if(stop_flag && !zdnum)
    {
